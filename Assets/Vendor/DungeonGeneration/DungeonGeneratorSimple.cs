...
namespace SimpleDungeon
{
    ...
    public class DungeonGeneratorSimple : MonoBehaviour
    {
        ...
        public void SpawnDungeon()
        {
            foreach(KeyValuePair<Vector3Int, TileType> kv in dungeon)
            {
                GameObject obj = null;
                switch (kv.Value) { 
                    case TileType.Floor: obj = Instantiate(floorPrefab, kv.Key, Quaternion.identity, transform); break;
                    case TileType.Wall: obj = Instantiate(wallPrefab, kv.Key, Quaternion.identity, transform); break;
                }
        ...
        public bool CanRoomFitInDungeon(Room room)
        {
            for (int x = room.minX-1; x <= room.maxX+1; x++)
            {
                ...
                {
                    if (dungeon.ContainsKey(new Vector3Int(x, y, 0))) { return false; }
                }
            }
            return true;
        }
    ...
    public class Room
    {
        ...
        {
            return new Vector3Int(Random.Range(minX,maxX+1), Random.Range(minY, maxY + 1), 0);
        }
    }
}